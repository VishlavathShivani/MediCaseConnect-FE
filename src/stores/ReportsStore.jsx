...
const useReportsStore = create((set, get) => ({
  ...
  isLoading: false,
  error: null,
  currentFilters: {}, // Store current filters for pagination
  pagination: {
    page: 1,
    limit: 10,
    totalCount: 0,
  ...
  // Search with filters
  fetchReports: async (filters = {}, page = null) => {
    set({ isLoading: true, error: null });
    
    // Use provided page or current page from pagination
    const currentPage = page || get().pagination.page;
    
    // Store filters for pagination use
  ...
  setPage: (page) => {
    ...
    // Fetch reports with current filters and new page
    get().fetchReports(get().currentFilters, page);
  },
